...
export HAVE_FEATURE="$HAVE_AES $HAVE_SSE2 $HAVE_SSE3 $HAVE_SSSE3 $HAVE_AVX $HAVE_AVX2 $HAVE_AVX512F"
	...
	(cd ../Native/libcryptonote && make clean && make) && mv ../Native/libcryptonote/libcryptonote.so "$OutDir"
		(cd ../Native/libethash && make clean && make) && mv ../Native/libethash/libethash.so "$OutDir"
		(cd ../Native/libgrhash && make clean && make) && mv ../Native/libgrhash/libgrhash.so "$OutDir"
		(cd ../Native/libkawpow && make clean && make) && mv ../Native/libkawpow/libkawpow.so "$OutDir"
		(cd ../Native/liblyrahash && make clean && make) && mv ../Native/liblyrahash/liblyrahash.so "$OutDir"
        (cd ../Native/libminotaurx && make clean && make) && mv ../Native/libminotaurx/libminotaurx.so "$OutDir"
		(cd ../Native/libmultihash && make clean && make) && mv ../Native/libmultihash/libmultihash.so "$OutDir"
		(cd ../Native/librandomx && make clean && make) && mv ../Native/librandomx/librandomx.so "$OutDir"
